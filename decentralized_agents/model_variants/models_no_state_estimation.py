...
class models_no_state_est(torch.nn.Module):
    ...
    """
        ...
        with torch.no_grad():
            ...
            h_t = threshold - torch.max(x_state)
            ...
            for node_idx in range(self.number_of_nodes):
                ...
                h_next = threshold - x_state[node_idx][0]
                #print(h_next)
        ...
        if mask.sum() == 0:
            ...
            for node in range(50):
                if node in neighbors:
                    if (threshold - x_state[node]) < min_val:
                        min_val=threshold - x_state[node]
                        min_node=node
            mask[min_node] = 1
