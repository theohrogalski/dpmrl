...
class models_extra_attention(torch.nn.Module):
    ...
    def __init__(self, number_of_nodes):
        ...
        self.graph_attention_1 = GAT(in_channels=6, hidden_channels=8, num_layers=2, out_channels=6)
        self.graph_attention_2 = GAT(in_channels=6, hidden_channels=8, num_layers=2, out_channels=6)
        self.multihead = nn.MultiheadAttention(embed_dim=6, num_heads=1)
        self.transform_two = TransformerConv(6, 5, heads=1)
    ...
    # Now state is [50, 6] -> (Obs + Topology + Prediction)
        ...
        x = self.graph_attention_1(x_enriched, edge_index)
        #print(f"x shape here is {x.shape}")

        x,_=self.multihead(x,x,x)
        
        x=self.transform_two(x,edge_index=edge_index)
        #print(f"x shape here is {x.shape}")
